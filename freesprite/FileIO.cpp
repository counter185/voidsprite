...
Layer* readMarioPaintSRM(PlatformNativePathString path, uint64_t seek)
{
    ...
    if (f != NULL) {
        ...
        try {
            ...
            l->palette = g_paletteByName("Mario Paint")->toRawColorList();
...
Layer* readXComSPK(PlatformNativePathString path, uint64_t seek)
{
    ...
    if (f != NULL) {
        ...
        ret->palette = g_paletteByName(PALETTE_DEFAULT)->toRawColorList();
        ret->name = "SPK Layer";
        uint32_t* pxd = ret->pixels32();
...
Layer* readSR8(PlatformNativePathString path, uint64_t seek)
{
    ...
    if (f != NULL) {
        ...
        l->palette = pal;*/
        IPalette* palette = g_paletteByName("Slim Render (8-bit)");
        l->palette = palette->toRawColorList();
...
void g_setupIO() {
    ...
    g_fileExporters.push_back(
        exAnymapPGM = FileExporter::flatExporter("Portable Graymap PGM", ".pgm", TL("vsp.export.pgm"), &writeAnymapPGM, FORMAT_RGB | FORMAT_PALETTIZED)
            ->buildParameters({
    ...
    g_fileImporters.push_back(FileImporter::sessionImporter("GraphicsGale", ".gal", &readGAL, NULL));
