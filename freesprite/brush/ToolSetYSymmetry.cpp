...
{
	editor->ssne.symmetryEnabled[1] = true;
	editor->ssne.symmetryPositions.y = pos.y;
	mouseHeld = true;
	lastEditor = editor;
	clickTimer.start();
}
...
{
	editor->ssne.symmetryEnabled[1] = true;
	editor->ssne.symmetryPositions.y = to.y;
}
...
{
	editor->ssne.symmetryEnabled[1] = !editor->ssne.symmetryEnabled[1];
}
...
void ToolSetYSymmetry::renderOnCanvas(XY canvasDrawPoint, int scale)
{
	...
	if (mouseHeld) {
		std::string tooltipString = frmt("{}{}", symYPos, symYMiddle ? ".5" : "");
		if (lastEditor != NULL && lastEditor->ssne.tileDimensions.y != 0) {
			tooltipString += frmt("\n(tile: {}{} / {})", symYPos % lastEditor->ssne.tileDimensions.y, symYMiddle ? ".5" : "", lastEditor->ssne.tileDimensions.y);
		}
		g_ttp->addTooltip({
