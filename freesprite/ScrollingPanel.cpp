...
void ScrollingPanel::handleInput(SDL_Event evt, XY gPosOffset) {
    ...
    if (scrollVertically && cevt.type == SDL_EVENT_MOUSE_BUTTON_DOWN && cevt.button.button == SDL_BUTTON_LEFT && mouseInVerticalScrollBar() && shouldVerticalScrollbarRender()) {
        draggingVerticalScrollbar = true;
    }
    else if (scrollHorizontally && cevt.type == SDL_EVENT_MOUSE_BUTTON_DOWN && cevt.button.button == SDL_BUTTON_LEFT && mouseInHorizontalScrollBar() && shouldHorizontalScrollbarRender())  {
