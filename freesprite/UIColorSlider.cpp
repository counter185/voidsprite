...
void UIColorSlider::render(XY pos)
{
	...
	gradientVerts[0].color = SDL_Color{ (uint8_t)((colorMin >> 16) & 0xff), (uint8_t)((colorMin >> 8) & 0xff), (uint8_t)((colorMin) & 0xff), 0xff };

	gradientVerts[1].position = { (float)pos.x + wxWidth, (float)pos.y };
	gradientVerts[1].color = SDL_Color{ (uint8_t)((colorMax >> 16) & 0xff), (uint8_t)((colorMax >> 8) & 0xff), (uint8_t)((colorMax) & 0xff), 0xff };

	gradientVerts[2].position = { (float)pos.x + wxWidth, (float)pos.y + wxHeight};
	gradientVerts[2].color = SDL_Color{ (uint8_t)((colorMax >> 16) & 0xff), (uint8_t)((colorMax >> 8) & 0xff), (uint8_t)((colorMax) & 0xff), 0xff };

	gradientVerts[3].position = { (float)pos.x, (float)pos.y + wxHeight };
	gradientVerts[3].color = SDL_Color{ (uint8_t)((colorMin >> 16) & 0xff), (uint8_t)((colorMin >> 8) & 0xff), (uint8_t)((colorMin) & 0xff), 0xff };
