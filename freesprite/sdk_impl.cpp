...
VSPBrush* impl_registerBrush(const char* name, const char* tooltip, bool wantDoublePrecision, void(*clickFunction)(VSPBrush* brush, VSPEditorContext* editor, int x, int y), void(*dragFunction)(VSPBrush* brush, VSPEditorContext* editor, int fromX, int fromY, int toX, int toY), void(*releaseFunction)(VSPBrush* brush, VSPEditorContext* editor, int x, int y))
{
    BrushExternal* b = new BrushExternal();
    b->name = name != NULL ? std::string(name) : "";
    b->tooltip = tooltip != NULL ? std::string(tooltip) : "";
    b->wantDoublePrecision = wantDoublePrecision;
    b->fClick = clickFunction;
    b->fDrag = dragFunction;
    b->fRelease = releaseFunction;
    g_pluginBrushes.push_back(b);
