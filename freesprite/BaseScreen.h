...
#define LALT_TO_SUMMON_NAVBAR if (evt.type == SDL_KEYDOWN && KEYCODE(evt) == SDL_SCANCODE_LALT) { if (!navbar->focused) wxsManager.forceFocusOn(navbar); else wxsManager.forceUnfocus(); return; }
