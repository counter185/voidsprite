...
int main(int argc, char** argv) {
    ...
    // load templates
    g_templates = {new TemplateRPG2KCharset(), new TemplateRPG2KChipset(),    new TemplateRPG2KFaceset(),
                   new TemplateRPG2KSystem(),  new TemplateRPG2KBattleAnim(), new TemplateMC64x32Skin()};
    ...
    auto customTemplatePaths = joinVectors(
        {platformListFilesInDir(platformEnsureDirAndGetConfigFilePath() + convertStringOnWin32("templates/"), ".png"),
         platformListFilesInDir(platformEnsureDirAndGetConfigFilePath() + convertStringOnWin32("templates/"),
                                ".voidsn")});
    ...
    if (custom9SPatterns > 0) {
        g_addNotification(
            Notification(std::format("Loaded {} custom 9seg. patterns", custom9SPatterns), "", 4000, NULL, COLOR_INFO));
    }
    ...
    while (!screenStack.empty()) {
        ...
        if (!screenStack.empty()) {
            ...
            g_fnt->RenderString(screenStack[currentScreen]->getName(), g_windowW - (10 + statW), g_windowH - 55,
                                {255, 255, 255, 255}, 16);
        }
