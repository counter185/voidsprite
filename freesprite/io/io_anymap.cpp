...
Layer* readAnymapPPM(PlatformNativePathString path, uint64_t seek, OperationProgressReport* report)
{
    ...
    if (f.is_open()) {
        ...
        report->enterSection("Reading PPM...");
        std::string line;
        std::getline(f, line);
        if (line == "P3") {
            //text ppm
            ...
            report->enterSection("Reading pixels...");
            for (int y = 0; y < h; y++) {
                report->updateLastSection(frmt("Reading pixels ({}/{})", y+1, h));
                for (int x = 0; x < w; x++) {
                    u32 col = 0xFF000000;
...
{
    bool binary = params == NULL || !params->hasParam("ppm.binary") ? false : params->getBool("ppm.binary");
    FILE* f = platformOpenFile(path, PlatformFileModeWB);
    if (f != NULL) {
        fprintf(f, binary ? "P6\n" : "P3\n");
        fprintf(f, "# File generated by voidsprite\n");
