...
Panel* ParameterStore::generateVerticalUI(std::function<void()> onChangedCallback)
{
    ...
    for (auto& p : parameters) {
        ...
        switch (p.paramType) {
            ...
            case PT_STRING:
            {
                UITextField* tf = new UITextField(p.vStr);
                tf->position = XY{ 250, y };
                tf->onTextChangedCallback = [this, i, onChangedCallback](UITextField*, std::string text) {
                    parameters.at(i).vStr = text;
                    onChangedCallback();
                };
                panel->subWidgets.addDrawable(tf);
                break;
            }
            case PT_COLOR_L:
            case PT_INT:
